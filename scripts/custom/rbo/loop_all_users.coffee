...
config.resolve (
  ...
  processUser = (user, done) ->
    ...
    #Missing Parent
    if !user.parentId
      missingParent.push {userId: user._id.toString(), name: user.name, clientId: user.clientId, email: user.email}

    #Root Parent
    if user._id.toString() is user.parentId?.toString()
      rootParent.push {userId: user._id.toString(), name: user.name, clientId: user.clientId, email: user.email}
...
# For each user, do stuff
    ...
    console.log "missingParent: ", missingParent.length
    console.log "rootParent: ", rootParent.length
