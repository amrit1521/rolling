APP = window.APP
APP.Controllers.controller('AdminEmailsHuntCatalog', ['$scope', '$routeParams', '$location', 'TenantEmail', 'User', ($scope, $routeParams, $location, TenantEmail, User) ->
  ...
  $scope.sendTestEmail = (email) ->
    ...
    if result
      ...
      TenantEmail.adminSendTest params, (rsp) ->
        alert "Hunt Catalog email test sent successfully"
      ,
      (err) ->
        if err.error
