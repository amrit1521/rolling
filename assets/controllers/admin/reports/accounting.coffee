APP = window.APP
APP.Controllers.controller('AdminReportsAccounting', ['$scope', '$rootScope', '$location', '$modal', '$routeParams','Storage', 'Report', 'View', 'Purchase', ($scope, $rootScope, $location, $modal, $routeParams, Storage, Report, View, Purchase) ->

  $scope.init = () ->
    $scope.GRID_SELECTOR_ID = '#reportsAccountingGrid'
    ...
    $scope.reportTypes = [
      { name: "All", value: "all" }
      { name: "Quickbooks Summary", value: "quickbooks" }
      { name: "Quickbooks Detailed", value: "quickbooks_detailed" }
      { name: "Outfitters All", value: "outfitters_all" }
      { name: "Outfitters Owed", value: "outfitters_owed" }
      { name: "Outfitters Paid", value: "outfitters_paid" }
      { name: "Vendors All", value: "vendors_all" }
      { name: "Vendors Owed", value: "vendors_owed" }
      { name: "Vendors Paid", value: "vendors_paid" }
    ]
  ...
  $scope.setItems = (results) ->
    items = []
    for item in results
      #Filter out data.  Better to do this on server side, but for now do it here.
      if $scope.reportType.indexOf("outfitter") > -1
        continue if item.huntCatalogCopy.type is "product" or item.huntCatalogCopy.outfitter_name.indexOf("Rolling Bones") > -1
  ...
  $scope.setupGrid = (dataSource) ->
    ...
    gridColumns.push { field: 'amount', title: 'Initial Deposit', hidden: false, aggregates: ["sum"], template: TEMPLATE_formatMoney('amount'), footerTemplate: TEMPLATE_Sum("Initial Deposit") }
    ...
    gridColumns.push { field: 'rbo_repTotal_applied', title: 'Rep Total Applied', hidden: true, aggregates: ["sum"], template: TEMPLATE_formatMoney('rbo_repTotal_applied'), footerTemplate: TEMPLATE_Sum("Rep Total") }
    ...
    gridColumns.push { field: 'commissionsPaid', title: 'Comm Paid Date', hidden: true, template: TEMPLATE_formatDate('commissionsPaid')}
    gridColumns.push { field: 'qb_invoice_recorded', title: 'QB Invoice Recorded', hidden: false }
    ...
    gridColumns.push { field: 'qb_expense_payment_paid_date', title: 'QB Expense Last Payment Date', hidden: true, template: TEMPLATE_formatDate('qb_expense_payment_paid_date')}
    ...
    gridColumns.push { field: 'rbo_rep0_name', title: 'AAA', hidden: true, aggregates: ["count", "sum"], groupHeaderTemplate: TEMPLATE_CommR0}
    ...
    reportShowColumns = []
    if $scope.reportType is "quickbooks"
      reportShowColumns = ['createdAt', 'huntCatalogCopy.title', 'invoiceNumber','user_name','amount','qb_invoice_recorded','qb_expense_bill_recorded','qb_invoice_payment_reconciled','qb_expense_payment_paid','qb_expense_payment_paid_date','qb_expense_balance_due','start_hunt_date','huntCatalogCopy.outfitter_name','huntCatalogCopy.type','edit','paymentUsed']
    else if $scope.reportType is "quickbooks_detailed"
      reportShowColumns = ['createdAt', 'huntCatalogCopy.title', 'invoiceNumber','TOTAL_PRICE','amount','amountPaid','status','remainingDepositToSend','qb_invoice_recorded','qb_expense_bill_recorded','qb_invoice_payment_reconciled','qb_expense_payment_paid','qb_expense_payment_paid_date','qb_expense_balance_due','start_hunt_date','huntCatalogCopy.outfitter_name','huntCatalogCopy.type','edit','paymentUsed']
    else if $scope.reportType is "outfitters_all"
      reportShowColumns = ['createdAt', 'huntCatalogCopy.title', 'invoiceNumber','user_name','amount','commission','remainingDepositToSend','qb_invoice_recorded','qb_expense_bill_recorded','qb_invoice_payment_reconciled','qb_expense_payment_paid','qb_expense_payment_paid_date','qb_expense_balance_due','start_hunt_date','huntCatalogCopy.outfitter_name','huntCatalogCopy.type','edit','paymentUsed']
    else if $scope.reportType isnt "all"
      reportShowColumns = ['createdAt', 'huntCatalogCopy.title', 'invoiceNumber','user_name','amount','qb_expense_bill_recorded','qb_expense_payment_paid_date','qb_expense_balance_due','start_hunt_date','huntCatalogCopy.outfitter_name','huntCatalogCopy.type','edit','paymentUsed']

    if $scope.reportType isnt "all"
      for gColumn in gridColumns
        if reportShowColumns.indexOf(gColumn.field) > -1
    ...
    angular.element($scope.GRID_SELECTOR_ID).data('kendoGrid').dataSource.read()
