APP = window.APP
APP.Controllers.controller('HuntsWyomingAntelope', ['$scope', '$location', '$routeParams', 'Hunt', 'HuntChoice', 'HuntOption', 'Storage', ($scope, $location, $routeParams, Hunt, HuntChoice, HuntOption, Storage) ->
  $scope.choice = {}
  $scope.hunts = {}
  $scope.types = {}
  ...
  $scope.setupWatch = (key, data) ->
    ...
    $scope.$watch 'choice.' + key, (newValue, oldValue) ->
      ...
      $scope.choice[typeName] = $scope.types?[typeName][0] if $scope.types?[typeName]?[0]
  ...
  $scope.setChoice = (areaNum, data) ->
    ...
    if not $scope.choices or not $scope.hunts?[huntName]?.length
      ...
      if $scope.types?[typeName]?.length
